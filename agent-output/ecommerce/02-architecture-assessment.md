...
```text
Azure Front Door (WAF) → App Service (P1v4 zone-redundant)
                      → SQL Database (S3) with Private Endpoint
                      → Redis Cache (C2) for sessions
                      → Cognitive Search (S1) for products
                      → Service Bus Premium for order processing
                      → Functions (EP1) for async workflows
```
...
### 💰 Cost Optimization Assessment (7/10)
