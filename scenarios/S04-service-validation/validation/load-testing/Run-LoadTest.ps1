<#
.SYNOPSIS
    Executes Azure Load Testing for S04 Service Validation
...
function Get-DeploymentInfo {
    ...
    # Find resource group with S04 deployment
    $rgPattern = "rg-s04-validation-*"
    $rgs = az group list --query "[?starts_with(name, 'rg-s04-validation-')].name" -o tsv
    
    if (-not $rgs) {
        throw "No S04 deployment found. Please deploy infrastructure first."
    }
...
function New-LoadTestResource {
    ...
    $testResourceName = "lt-s04-validation"
    ...
    if ($PSCmdlet.ShouldProcess($testResourceName, "Create Load Testing resource")) {
        az load test create `
            ...
            --location $Location `
            --tags "Project=S04-Service-Validation" "Environment=test" | Out-Null
...
function Start-LoadTest {
    ...
    $testId = "s04-$TestName-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
    ...
    az load test create `
        ...
        --resource-group $ResourceGroup `
        --display-name "S04 $TestName Test" `
    ...
    az load test-run create `
        ...
        --load-test-resource $TestResourceName `
        --resource-group $ResourceGroup `
        --display-name "S04 $TestName - $(Get-Date -Format 'yyyy-MM-dd HH:mm')" | Out-Null
...
try {
    Write-Banner "Azure Load Testing - S04 Service Validation"
